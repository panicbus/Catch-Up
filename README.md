...
- **Local-story deprioritization** — set your home city in Settings and topic/entity channels (e.g. a *Wildfires* channel), plus a *Politics* channel specifically, quietly deprioritize local stories about places far from you, so a small distant town's story doesn't crowd out the ones near you. Other broad category channels are unaffected. Fully offline — no external geocoding call.
...
**Local-story deprioritization (topic/entity channels, plus the Politics category channel).** When you've set a home city in Settings, Stage 1's score gets one more input: the story's title/snippet is scanned for place names against a bundled city gazetteer, and a story whose nearest mentioned place is far from home gets a mild penalty (a strong on-topic story survives regardless of distance; only borderline ones get tipped out). Every other broad category channel never applies this — they're supposed to show geographic variety, and World specifically has no keyword list of its own to fall back on if a story lacked a section tag, so applying the penalty there would drop ordinary far-away world news for being far away.
